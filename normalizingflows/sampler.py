...
def sample_anomalies(model, n_samples=1000, mu=0, sigma=1, factor=2, sample_shape=2):
    clip_a_l = -np.inf
    ...
    generated_anomalies = model.flow.bijector.forward(samples)
    return generated_anomalies


def sample_normals(model, n_samples=3000, mu=0, sigma=1, factor=2, sample_shape=2):
    ...
    samples = model.flow.bijector.forward(samples)
    return samples